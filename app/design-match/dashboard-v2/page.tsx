/**
 * Phase 5.0 — /design-match/dashboard-v2
 *
 * Reconstruction pixel-perfect totalement indépendante de v1 et du
 * vrai /dashboard. Mesures extraites par analyse pixel Python/PIL
 * directement depuis docs/design-system/mockups/dashboard.png.
 *
 * Toutes les valeurs sont en pixels exacts, pas en classes Tailwind
 * approximatives. Inline styles + tokens locaux.
 *
 * Mesures de référence (extraites à pixel près) :
 *   Canvas        1536 × 1024
 *   Sidebar       0 → 279 px (largeur 280)
 *   Topbar        0 → 91 px  (hauteur 91, on prend 90 px)
 *   Score card    y=91 → 306  hauteur 215 px
 *   Gap H→R       22 px
 *   Roadmap       y=328 → 494 hauteur 166 px
 *   Gap R→K       12 px
 *   KPI strip     y=506 → 634 hauteur 128 px
 *   Gap K→B       16 px
 *   Bottom row    y=650 → 910 hauteur 260 px
 *   Gap B→C       24 px
 *   Coach CTA     y=934 → 983 hauteur 49 px
 *
 * Couleurs (sample RGB pixel) :
 *   Navy          rgb(2,31,96)   = #021F60
 *   Page bg       rgb(245,247,250) ≈ #F5F7FA
 *   Card bg       rgb(254,254,254) = #FEFEFE
 *   Border        rgb(229,233,240) = #E5E9F0 (subtil)
 *   Primary blue  #2563EB (CTA links)
 *   Success       #10A37F (deltas + coral icons + chart-green)
 *
 * Aucun composant @/components réutilisé. Aucun import @/lib hors
 * Next.js core. Page autonome.
 */

export const metadata = {
  title: "Design Match v2 — Dashboard",
...
const SHADOW = {
  card: "0 1px 2px rgb(15 23 42 / 0.04), 0 6px 16px -8px rgb(15 23 42 / 0.08)",
  navy: "0 1px 3px rgb(15 23 42 / 0.06), 0 12px 32px -12px rgb(2 31 96 / 0.20)",
};

// ── Hauteurs mesurées exactes ──────────────────────────────────────
const H = {
  topbar: 90,
  scoreCard: 215,
  roadmap: 166,
...
export default function DesignMatchDashboardV2() {
  return (
    <div
      style={{
        display: "flex",
    ...
    >
      ...
      <div style={{ marginLeft: 280, flex: 1, display: "flex", flexDirection: "column" }}>
        <Topbar />
        <main style={{ flex: 1, padding: "0 40px 32px 40px" }}>
...
/* ═════════════════════════════════════════════════════════════════ */
/*  SIDEBAR — 280 px, fond blanc, 4 sections + premium card           */
/* ═════════════════════════════════════════════════════════════════ */
...
function Sidebar() {
  return (
    ...
    >
      {/* Logo */}
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "24px 24px 20px 24px" }}>
        <span
          style={{
            display: "inline-flex",
            alignItems: "center",
      ...
      {/* Nav */}
      <nav style={{ flex: 1, overflowY: "auto", padding: "0 12px 0 12px" }}>
        ...
        <NavSection title="CROISSANCE">
          <NavItem label="Épargne" iconPath="M21 11h-1a4 4 0 0 0-4-4h-4a8 8 0 0 0-8 8 6 6 0 0 0 6 6h2v-3h4v3h2a6 6 0 0 0 4-2v-2h2v-6z|M16 15h0" />
    ...
    >
      <div style={{ display: "flex", height: "100%", justifyContent: "space-between", alignItems: "stretch" }}>
        ...
        <div style={{ position: "relative", flexShrink: 0, width: 130, height: 130 }}>
          ...
          />
          <svg viewBox="0 0 100 100" width={130} height={130} style={{ position: "relative" }}>
    ...
    >
      <span
        style={{
          ...
          width: 36,
          height: 36,
          backgroundColor: isToday ? C.primaryBg : bg,
...
/* ═════════════════════════════════════════════════════════════════ */
/*  KPI ROW — 128 px (corrigé vs v1 qui était à 110)                 */
/* ═════════════════════════════════════════════════════════════════ */
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", marginTop: 14, gap: 16 }}>
        <div style={{ position: "relative", flexShrink: 0, width: 124, height: 124 }}>
      ...
      <div style={{ marginTop: 8 }}>
        <svg viewBox={`0 0 ${W} ${HH}`} width="100%" height={HH}>
          ...
          {[0, 25, 50, 75, 100].map((v) => {
            ...
            return <line key={v} x1={PAD.left} x2={W - PAD.right} y1={y} y2={y} stroke={C.border} strokeWidth={0.5} />;
          })}
          {[25, 50, 75, 100].map((v) => {
            const y = PAD.top + ((100 - v) / 100) * innerH;
            return (
