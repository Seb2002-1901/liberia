...
import type { ActionErrorKey } from "@/lib/i18n/action-errors";
 ...
 *
 * Returns `{ ok: true }` when the user has access, `{ ok: false, reasonKey }`
 * otherwise. The caller resolves the key against the user's locale via
 * `getActionErrors()`.
 */
export async function requirePremiumAccess(
  supabase: SupabaseClient,
  userId: string,
): Promise<{ ok: true } | { ok: false; reasonKey: ActionErrorKey }> {
  ...
  if (!status) {
    return { ok: false, reasonKey: "premiumRequired" };
  }
  return { ok: false, reasonKey: "premiumLapsed" };
}
